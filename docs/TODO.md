...
- [ ] **M1b** 🟢 **Type alias transitive chains single-hop only** — `A: type = B; B: type = i64` resolves `A` to `B` (not `i64`). `resolveTypeAnnotationInScope` stops at one level. Add loop in `resolveAliasTarget` or store resolved chain end in `type_alias` payload.
- [ ] **M1c** 🟢 **Duplicated `Primitive→RT` switch** — `resolver.zig:164-170` and `:271-276` have identical logic. Extract to a shared helper.
