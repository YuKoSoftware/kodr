...
> **Session bookmark** (v0.53.25, 2026-04-26). P2 done — transitive cache invalidation, atomic ZON writes, full cycle paths. ⬅ **RESUME HERE: Phase 3 (P3)** — LSP reuses pipeline via `runPasses(stop_after:)` (needs design work); or **Phase 3 (P4)** — rewrite `typeToZig` as pure function (independent codegen cleanup, no design needed); or **Phase 4 (X1)** — table-driven CLI parser (independent).
