...
- [ ] **M20b** 🟢 **`canonicalUnionRef` calls `typeToZig` twice per member** — once as sort key, once for output. Redundant work on every union emission. Trivially cacheable. The E9005 fix pushed type-alias codegen to use `type_id` → `zigOfRT`; `canonicalUnionRef` is the remaining AST-walk hot site.
