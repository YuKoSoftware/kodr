    ...
    switch (match_type) {
        ...
        .named => |name| {
            ...
            if (enum_sig) |esig| {
                ...
                _ = try self.ctx.reporter.reportFmt(.match_non_enum_needs_else, self.ctx.nodeLoc(match_node),
...
fn reportMissingArms(self: *TypeResolver, missing: []const []const u8, match_node: *parser.Node) !void {
    ...
    _ = try self.ctx.reporter.reportFmt(.match_non_exhaustive, self.ctx.nodeLoc(match_node), "{s}", .{buf.items});
}
...
pub fn validateType(self: *TypeResolver, node: *parser.Node, scope: *Scope, rctx: ResolveCtx) anyerror!void {
    switch (node.*) {
        .type_named => |type_name| {
            ...
            if (!is_known) {
                ...
                if (types.Primitive.fromName(type_name) == .this or types.Primitive.fromName(type_name) == .self_deprecated) {
                    _ = try self.ctx.reporter.reportFmt(.this_outside_struct, self.ctx.nodeLoc(node),
                ...
                _ = try self.ctx.reporter.reportFmt(.unknown_type, self.ctx.nodeLoc(node), "unknown type '{s}'{s}", .{ type_name, hint });
            }
        },
        ...
        .type_generic => |g| {
            ...
            if (!is_known) {
                _ = try self.ctx.reporter.reportFmt(.unknown_generic_type, self.ctx.nodeLoc(node), "unknown generic type '{s}'", .{g.name});
            }
        ...
        .type_tuple_named => {
            _ = try self.ctx.reporter.reportFmt(.anonymous_tuple_not_allowed, self.ctx.nodeLoc(node), "anonymous tuple types are not allowed — define a named type alias with 'const'", .{});
        },
        else => {},
    }
}
...
pub fn checkAssignCompat(self: *TypeResolver, expected: RT, actual: RT, node: *parser.Node) !void {
    ...
    _ = try self.ctx.reporter.reportFmt(.type_mismatch, self.ctx.nodeLoc(node), "type mismatch: expected '{s}', got '{s}'",
        ...
        if (param_type == .primitive and param_type.primitive == .string) {
            if (arg_type == .slice) {
                if (arg_type.slice.* == .primitive and arg_type.slice.primitive == .u8) {
                    _ = try self.ctx.reporter.reportFmt(.bytes_str_implicit, self.ctx.nodeLoc(node), "cannot pass '[]u8' as 'str' — use string.fromBytes() for explicit conversion",
                        .{});
                }
            }
        }
        ...
        if (param_type == .slice) {
            if (param_type.slice.* == .primitive and param_type.slice.primitive == .u8) {
                if (arg_type == .primitive and arg_type.primitive == .string) {
                    _ = try self.ctx.reporter.reportFmt(.str_bytes_implicit, self.ctx.nodeLoc(node), "cannot pass 'str' as '[]u8' — use string.toBytes() for explicit conversion",
                        .{});
                }
            }
        }
...
pub fn checkBlueprintConformance(self: *TypeResolver, s: parser.StructDecl, loc: ?errors.SourceLoc) anyerror!void {
    ...
    for (s.blueprints, 0..) |bp_name, i| {
        for (s.blueprints[0..i]) |prev| {
            if (std.mem.eql(u8, bp_name, prev)) {
                _ = try self.ctx.reporter.reportFmt(.duplicate_blueprint_impl, loc, "struct '{s}' lists blueprint '{s}' more than once", .{ s.name, bp_name });
            }
        }
    }
    ...
    for (s.blueprints) |bp_name| {
        ...
        const bp_sig: declarations.BlueprintSig = blk: {
            ...
            _ = try self.ctx.reporter.reportFmt(.unknown_blueprint, loc, "unknown blueprint '{s}'", .{bp_name});
            continue;
        };
        ...
        for (bp_sig.methods) |bp_method| {
            const struct_method: declarations.FuncSig = blk: {
                ...
                _ = try self.ctx.reporter.reportFmt(.blueprint_not_implemented, loc, "struct '{s}' does not implement '{s}' required by blueprint '{s}'",
            ...
            if (struct_method.params.len != bp_method.params.len) {
                _ = try self.ctx.reporter.reportFmt(.blueprint_param_count, loc, "method '{s}' in struct '{s}' has {d} parameter(s), blueprint '{s}' requires {d}",
            ...
            for (struct_method.params, bp_method.params) |sp, bp| {
                if (!resolver_mod.typesMatchWithSubstitution(sp.type_, bp.type_, bp_name, s.name)) {
                    _ = try self.ctx.reporter.reportFmt(.blueprint_param_type, loc, "method '{s}' in struct '{s}' does not match blueprint '{s}': parameter type mismatch",
            ...
            if (!resolver_mod.typesMatchWithSubstitution(struct_method.return_type, bp_method.return_type, bp_name, s.name)) {
                _ = try self.ctx.reporter.reportFmt(.blueprint_return_type, loc, "method '{s}' in struct '{s}' does not match blueprint '{s}': return type mismatch",
                    .{ bp_method.name, s.name, bp_name });
            }
        }
    }
