    ...
    switch (node.*) {
        ...
        .for_stmt => |f| {
            ...
            if (f.is_tuple_capture) {
                ...
                for (f.captures) |v| try scope.define(v, true);
            } else {
                // Each capture maps to an iterable
                for (f.captures, f.iterables) |v, iter| {
                    try scope.define(v, self.inferIterableElemPrimitive(iter, scope));
                }
            }
        ...
        .identifier => |name| {
            if (scope.getState(name)) |state| {
                if (state.state == .moved) {
                    _ = try self.ctx.reporter.reportFmt(.use_of_moved_value, self.ctx.nodeLoc(node), "use of moved value '{s}' — consider using @copy()", .{name});
                }
                    ...
                    if (!state.is_primitive) {
                        ...
                        if (field_is_prim) |is_prim| {
                            if (!is_prim) {
                                ...
                                _ = try self.ctx.reporter.reportFmt(.cannot_move_field, self.ctx.nodeLoc(node), "cannot move field '{s}' out of '{s}' — structs are atomic ownership units — consider using @copy()",
                                    .{ f.field, obj_name });
                            }
