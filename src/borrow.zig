...
pub const BorrowChecker = struct {
    ...
    pub fn checkNotMutablyBorrowedPath(self: *BorrowChecker, name: []const u8, field: ?[]const u8) !void {
        for (self.active_borrows.items) |b| {
            ...
            if (field) |f| {
                _ = try self.ctx.reporter.reportFmt(.use_while_borrowed, loc, "cannot use '{s}.{s}' while it is mutably borrowed — consider borrowing with const&", .{ name, f });
            } else {
                _ = try self.ctx.reporter.reportFmt(.use_while_borrowed, loc, "cannot use '{s}' while it is mutably borrowed — consider borrowing with const&", .{name});
            }
            return;
        }
    }
        ...
        for (self.active_borrows.items) |existing| {
            ...
            if (is_mutable or existing.is_mutable) {
                ...
                const hint: []const u8 = if (!is_mutable)
                    ...
                    "";
                _ = try self.ctx.reporter.reportFmt(.borrow_conflict, loc, "cannot borrow '{s}' as {s}: already borrowed as {s}{s}",
                    .{
                        label,
