        ...
        switch (node.*) {
            ...
            .var_decl => |v| {
                if (v.mutability == .mutable) {
                    _ = try self.reporter.reportFmt(.module_level_var, loc, "module-level 'var' is not allowed — use 'const' for module-level declarations", .{});
                } else {
        ...
        for (f.params) |param_node| {
            if (param_node.* == .param) {
                if (param_node.param.default_value != null) {
                    ...
                    _ = try self.reporter.reportFmt(.default_before_required, loc, "parameters with defaults must come after all required parameters in '{s}'", .{f.name});
                    break;
                }
            }
        }
    ...
    fn collectStruct(self: *DeclCollector, s: parser.StructDecl, loc: ?errors.SourceLoc) anyerror!void {
        ...
        for (s.members) |member| {
            ...
            if (member.* == .var_decl and member.var_decl.mutability == .mutable) {
                _ = try self.reporter.reportFmt(.var_in_struct, loc, "mutable 'var' not allowed in struct '{s}' — use 'const' for static declarations", .{s.name});
            }
        ...
        for (fields.items, 0..) |field, i| {
            if (isReservedTypeName(field.name)) {
                _ = try self.reporter.reportFmt(.field_name_conflicts_type, loc, "field name '{s}' conflicts with type name — choose a different name", .{field.name});
            }
            // Check for duplicate field names
            for (fields.items[0..i]) |prev| {
                if (std.mem.eql(u8, field.name, prev.name)) {
                    _ = try self.reporter.reportFmt(.duplicate_field, loc, "duplicate field '{s}' in struct '{s}'", .{ field.name, s.name });
                    break;
                }
            }
        }
            ...
            error.DuplicateUnionMember => {
                ...
                if (type_node != null and type_node.?.* == .type_union) {
                    if (types.findDuplicateUnionMember(self.allocator, type_node.?.type_union)) |dup_name| {
                        _ = try self.reporter.reportFmt(.duplicate_union_type, loc, "duplicate type '{s}' in union — each type may appear only once", .{dup_name});
                        return;
                    }
                }
                _ = try self.reporter.reportFmt(.duplicate_union_type, loc, "duplicate type in union — each type may appear only once", .{});
            },
            else => return err,
        }
    }
            ...
            if (member.* == .enum_variant) {
                ...
                for (variants.items) |prev| {
                    if (std.mem.eql(u8, member.enum_variant.name, prev)) {
                        _ = try self.reporter.reportFmt(.duplicate_enum_variant, loc, "duplicate variant '{s}' in enum '{s}'", .{ member.enum_variant.name, e.name });
                        break;
                    }
                }
