        ...
        switch (node.*) {
            ...
            .block => try self.checkNode(node, scope),
                ...
                if (try self.exprReturnsUnion(node)) |is_error| {
                    ...
                    _ = try self.ctx.reporter.reportFmt(.discarded_union_return, self.ctx.nodeLoc(node),
                ...
                if (f.object.* == .identifier) {
                    ...
                    if (scope.isTracked(name)) |uvar| {
                        if (!uvar.handled) {
                            ...
                            _ = try self.ctx.reporter.reportFmt(.unsafe_unwrap, self.ctx.nodeLoc(node), "unsafe unwrap of {s} union '{s}' — check with 'is' or 'match' first",
                                .{ kind, name });
                        }
                    }
                }
        ...
        for (scope.base.currentFrameBindings()) |binding| {
            ...
            if (!uvar.handled) {
                ...
                const loc: ?errors.SourceLoc = if (uvar.line > 0) blk: {
                    ...
                    null;
                _ = try self.ctx.reporter.reportFmt(.unhandled_union, loc, "unhandled {s} union '{s}' — use if/match to handle before scope exit",
                    .{ kind, uvar.name });
            }
        }
    }
