...
fn resolveExprInner(self: *TypeResolver, node: *parser.Node, scope: *Scope, rctx: ResolveCtx) anyerror!RT {
    return switch (node.*) {
        ...
        .identifier => |id_name| {
            ...
            _ = try self.ctx.reporter.reportFmt(.unknown_identifier, self.ctx.nodeLoc(node), "unknown identifier '{s}'{s}", .{ id_name, hint });
            return RT.unknown;
        },
        ...
        .binary_expr => |b| {
            ...
            if (b.op == .eq or b.op == .ne) {
                if (l_is_str or r_is_str) {
                    _ = try self.ctx.reporter.reportFmt(.str_equality, self.ctx.nodeLoc(node),
            ...
            if (is_arithmetic and (l_is_str or r_is_str)) {
                _ = try self.ctx.reporter.reportFmt(.str_arithmetic, self.ctx.nodeLoc(node),
            ...
            if (b.op == .concat) {
                ...
                if (l_is_num or r_is_num) {
                    _ = try self.ctx.reporter.reportFmt(.concat_on_numeric, self.ctx.nodeLoc(node),
            ...
            if (left == .primitive and right == .primitive) {
                ...
                if (lp.isNumeric() and rp.isNumeric() and
                    ...
                    lp != rp)
                {
                    _ = try self.ctx.reporter.reportFmt(.mixed_numeric_types, self.ctx.nodeLoc(node),
        ...
        .unary_expr => |u| {
            ...
            if (u.op == .negate) {
                if (operand_type == .primitive and operand_type.primitive.isUnsigned()) {
                    _ = try self.ctx.reporter.reportFmt(.negate_unsigned, self.ctx.nodeLoc(node),
        ...
        .call_expr => |c| {
            ...
            if (c.callee.* == .identifier) {
                ...
                if (self.ctx.decls.symbols.get(name)) |sym| switch (sym) {
                    .@"struct", .type_alias => {
                        if (c.args.len > 0 and c.arg_names.len == 0) {
                            _ = try self.ctx.reporter.reportFmt(.struct_constructor_syntax, self.ctx.nodeLoc(node),
                ...
                if (c.arg_names.len > 0) {
                    _ = try self.ctx.reporter.reportFmt(.named_args_not_struct, self.ctx.nodeLoc(node),
                ...
                if (scope.lookup(name)) |t| {
                        ...
                        !self.isIncludedType(name))
                    {
                        ...
                        _ = try self.ctx.reporter.reportFmt(.not_callable, self.ctx.nodeLoc(node), "'{s}' is not callable — expected a function or constructor", .{name});
                        return t;
                    }
                }
                if (self.ctx.decls.symbols.get(name)) |sym| switch (sym) {
                    .func => |sig| {
                        ...
                        if (c.args.len < min_args or c.args.len > max_args) {
                            if (min_args == max_args) {
                                _ = try self.ctx.reporter.reportFmt(.arg_count_mismatch, self.ctx.nodeLoc(node),
                            ...
                            for (c.args, 0..) |arg, idx| {
                                if (!isComptimeKnown(self, arg, rctx)) {
                                    ...
                                    const is_type_param = if (idx < sig.params.len)
                                        ...
                                        false;
                                    if (is_type_param) {
                                        _ = try self.ctx.reporter.reportFmt(.compt_type_arg_required, self.ctx.nodeLoc(node),
                                            "compt function '{s}' expects a type argument, but '{s}' is a function parameter", .{ name, arg_name });
                                    } else {
                                        _ = try self.ctx.reporter.reportFmt(.compt_runtime_arg, self.ctx.nodeLoc(node),
        ...
        .index_expr => |i| {
            ...
            if (obj_type == .primitive) {
                ...
                if (tn == .bool or tn == .void) {
                    _ = try self.ctx.reporter.reportFmt(.cannot_index, self.ctx.nodeLoc(node), "cannot index into type '{s}'", .{tn.toName()});
                }
            }
            return RT.inferred;
        },
        ...
        .compiler_func => |cf| {
            ...
            const f = func orelse {
                _ = try self.ctx.reporter.reportFmt(.unknown_compiler_func, loc, "unknown compiler function '@{s}'", .{cf.name});
                return RT.unknown;
            };

            // @type is an internal desugaring artifact from `is` — reject outside if/elif
            if (f == .@"type" and !rctx.in_is_condition) {
                _ = try self.ctx.reporter.reportFmt(.compiler_func_needs_type, loc,
            ...
            if (cf.args.len < expected.min or cf.args.len > expected.max) {
                if (expected.min == expected.max) {
                    _ = try self.ctx.reporter.reportFmt(.compiler_func_arg_count, loc, "@{s} takes exactly {d} argument(s)", .{ cf.name, expected.min });
                } else {
                    _ = try self.ctx.reporter.reportFmt(.compiler_func_arg_count, loc, "@{s} takes {d} to {d} arguments", .{ cf.name, expected.min, expected.max });
                }
            }
            ...
            switch (f) {
                .hasField, .hasDecl, .fieldType => {
                    if (cf.args.len >= 2 and cf.args[1].* != .string_literal) {
                        _ = try self.ctx.reporter.reportFmt(.compiler_func_needs_string, loc, "@{s} requires a string literal as second argument", .{cf.name});
                    }
                },
                else => {},
            }
                ...
                .wrap, .sat, .overflow => {
                    if (cf.args.len >= 1 and cf.args[0].* == .binary_expr) {
                        ...
                        if (op != .add and op != .sub and op != .mul) {
                            _ = try self.ctx.reporter.reportFmt(.wrap_op_not_supported, loc, "@{s} only supports +, -, * operators — division and modulo have no wrapping equivalents", .{cf.name});
                        }
                    }
                },
                else => {},
        ...
        .array_literal => |elems| {
            ...
            for (elems) |elem| {
                ...
                {
                    _ = try self.ctx.reporter.reportFmt(.array_elem_type_mismatch, self.ctx.nodeLoc(elem),
...
fn validateCallStyle(
    ...
    if (sig.is_instance and called_on_type) {
        _ = try self.ctx.reporter.reportFmt(.instance_method_on_type, self.ctx.nodeLoc(node),
            "'{s}' is an instance method — call it on a value: 'value.{s}()'", .{ method_name, method_name });
    }
    if (!sig.is_instance and !called_on_type) {
        _ = try self.ctx.reporter.reportFmt(.static_method_on_value, self.ctx.nodeLoc(node),
